...
//		}
		...
		public T CreateAsset<T>(string path) where T : MonoBehaviour
		{
			...
			if (asset == null)
			{
                BaseLogger.LogError("Prefab {0} doesn't hava script: {1}", path, typeof(T).ToString());
			}
			return asset;
		}
		public GameObject CreateGameObject(string path)
		{
			...
			if (prefab == null)
			{
                BaseLogger.LogError("Can't create prefab: {0}", path);
			}
        ...
        public void PreloadAsset (AssetType type, string path, float life = -1f, int maxPreloadCount = 1, int growth = 1)
		{
        ...
        public T LoadAsset<T> (AssetType type, string assetName) where T : MonoBehaviour
		{
			GameObject go = LoadGameObject(type, assetName);
			T asset = go.GetComponent<T>();
			if (asset == null)
			{
                BaseLogger.LogError("Prefab {0} doesn't hava script: {1}", assetName, typeof(T).ToString());
			}
			return asset;
		}

        public GameObject LoadGameObject (AssetType type, string assetName)
		{
			GameObject go = assetManager.LoadAsset(type, assetName);
			if (go == null)
			{
                BaseLogger.LogError("Cannot load prefab: {0} - {1}", type, assetName);
			}
			return go;
		}
