...
namespace Base
{
	public class AssetManager
	{
        ...
        private Dictionary<AssetType, AssetPool> assetPoolDic;

		public AssetManager ()
		{
			secondDelay = new WaitForSeconds(1f);
            assetPoolDic = new Dictionary<AssetType, AssetPool>();

            var typeArr = Enum.GetValues(typeof(AssetType));
            foreach (object obj in typeArr)
            {
                AssetType type = (AssetType)obj;
                assetPoolDic.Add(AssetType.Monster, new AssetPool());
            }
		}
        ...
        public void PreloadAsset (AssetType type, string path, float life, int maxPreloadCount, int growth)
		{
			AssetPool pool = null;
        ...
        public GameObject LoadAsset(AssetType type, string assetName)
		{
			AssetPool pool = null;
			assetPoolDic.TryGetValue(type, out pool);
